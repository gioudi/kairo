...
namespace KAIROSV2.WebApp.Identity.Authorization
{
    internal class PermissionsPolicyProvider : IAuthorizationPolicyProvider
    {
        ...
        // Policies are looked up by string name, so expect 'parameters' (like permission)
        ...
        public Task<AuthorizationPolicy> GetPolicyAsync(string policyName)
        {
            if (policyName.StartsWith(POLICY_PREFIX, StringComparison.OrdinalIgnoreCase) &&
                Enum.TryParse<Permissions>(policyName.Substring(POLICY_PREFIX.Length), out var permissionId))
            {
                var policy = new AuthorizationPolicyBuilder();
                policy.AddRequirements(new PermissionsRequirement(permissionId));
